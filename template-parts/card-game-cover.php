<?php if (get_post_type() === 'mega-drive') : ?>
  <a href="<?php the_permalink(); ?>" class="md-cover game-cover">
    <?php the_post_thumbnail( 'md_cover', array( 'alt' => the_title_attribute( array( 'echo' => false, ) ), ) ); ?>
    <div class="entry-header">
      <?php
      the_title( '<p class="entry-title">', '</p>' );

      if ( 'post' === get_post_type() ) :
        ?>
        <div class="entry-meta">
          <?php
          megadriveme2020_posted_on();
          megadriveme2020_posted_by();
          ?>
        </div>
      <?php endif; ?>
    </div>
  </a>
<?php elseif (get_post_type() === 'mega-cd') : ?>
  <a href="<?php the_permalink(); ?>" class="mega-cd-cover game-cover">
    <?php the_post_thumbnail( 'md_cover', array( 'alt' => the_title_attribute( array( 'echo' => false, ) ), ) ); ?>
    <div class="entry-header">
      <?php the_title( '<p class="entry-title">', '</p>' );?>
    </div>
  </a>
<?php elseif (get_post_type() === '32x') : ?>
  <a href="<?php the_permalink(); ?>" class="32x-cover game-cover">
    <?php the_post_thumbnail( 'md_cover', array( 'alt' => the_title_attribute( array( 'echo' => false, ) ), ) ); ?>
    <div class="entry-header">
      <?php the_title( '<p class="entry-title">', '</p>' );?>
    </div>
  </a>
<?php elseif (get_post_type() === 'hardware') : ?>
  <a href="<?php the_permalink(); ?>" class="hardware-cover game-cover">
    <?php the_post_thumbnail( 'md_cover', array( 'alt' => the_title_attribute( array( 'echo' => false, ) ), ) ); ?>
    <div class="entry-header">
      <?php the_title( '<p class="entry-title">', '</p>' );?>
    </div>
  </a>
<?php endif; ?>
      